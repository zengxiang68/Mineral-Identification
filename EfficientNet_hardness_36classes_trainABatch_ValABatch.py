...
from EfficientNet_hardness import EfficientNet
from sampler import WeightedRandom_DistributedSampler
from load_data import valDataProvider, mineral_dataset_hardness
...
def train_and_val(train_loader, model, cost, optimizer, epochs):
    ...
    for i, (imgs, labels, hardnesses) in enumerate(train_loader):
        # train
        model.train()
        imgs = torch.autograd.Variable(imgs.to(device))
        labels = torch.autograd.Variable(labels.to(device))
        hardnesses = hardnesses.expand(1, batch_size).t().float()
        hardnesses = torch.autograd.Variable(hardnesses.to(device))

        output = model(imgs, hardnesses)
        ...
        if i % 40 == 0:
            logger.info("Loss : {:.3f}  acc1: {:.2f} acc5: {:.2f}".format(
                float(loss), float(acc1), float(acc5)))

        # validate
        model.eval()
        with torch.no_grad():  # avoid out of memory
            val_imgs, val_labels, val_hardnesses = valData.next()
            val_imgs = torch.autograd.Variable(val_imgs.to(device))
            val_labels = torch.autograd.Variable(val_labels.to(device))
            val_hardnesses = val_hardnesses.expand(1, batch_size*2).t().float()
            val_hardnesses = torch.autograd.Variable(val_hardnesses.to(device))

            output = model(val_imgs, val_hardnesses)
    ...
    if distributed:
        if args.local_rank == 0:
            ...
            logger.info("average loss : {:.3f}".format(ave_loss))
            logger.info("average acc1 : {:.2f}".format(ave_acc1))
    else:
        logger.info('-'*5)
        logger.info("for this epoch")
        logger.info("average loss : {:.3f}".format(ave_loss))
...
if __name__ == "__main__":
    ...
    if not torch.cuda.is_available():
        logger.error("Error, can't use GPU CUDA.")
        sys.exit(1)
    else:
    ...
    train_dataset = mineral_dataset_hardness(train_dir, training=True)
    val_dataset = mineral_dataset_hardness(val_dir, training=False)
    ...
    for epoch in range(epochs):
        ...
        if val_acc1 > acc1_best:
            ...
            pth_name = 'epoches_{}_acc1_{:.2f}_%_EfficientNet-b3_hardness.pth'\
